(function() {
  ...
  // ─── Lucide 图标图鉴（已抽到 js/icons.js，通过 window.ICONS 引用）───
...
function viewSummary(id) {
  ...
  // ─── 快速提交浮动按钮 ───
  var fabBtn = document.getElementById('fabApply');
  if (fabBtn) {
    fabBtn.addEventListener('click', function() { go('apply'); });
    // 仅员工角色显示
    function updateFab() {
      if (ME && ME.role !== 'hr' && CURRENT_PAGE === 'my') {
  ...
  // ─── 表单实时校验 ───
  function addFieldValidation(id, validator) {
    var el = document.getElementById(id);
    if (!el) return;
    el.addEventListener('blur', function() {
      var ok = validator(el.value);
      el.classList.toggle('field-error', !ok);
      el.classList.toggle('field-ok', ok && el.value.trim() !== '');
      var msg = el.parentNode.querySelector('.field-error-msg');
      if (msg) msg.style.display = (!ok && el.value.trim() !== '') ? 'block' : 'none';
    });
  ...
  // 申请表单校验（延迟绑定，等DOM渲染）
  setTimeout(function() {
    addFieldValidation('a-proj', function(v) { return v.trim().length >= 2; });
    addFieldValidation('a-date', function(v) { return !!v; });
    addFieldValidation('a-cost', function(v) { return v === '' || (parseFloat(v) >= 0 && !isNaN(v)); });
    addFieldValidation('a-goal', function(v) { return v.trim().length >= 20; });
