...
- 员工只能做这一个状态转换：`已通过`/`学习中` → `总结已提交`（即总结被退回后（学习中）可**编辑原总结并重新提交**：前端自动回填六个字段 + 显示退回原因，2026-09-23 加）
...
| 2026-09-23 | - | **新功能**：退回后编辑重新提交（学习中可修改原总结重提，前端自动回填+退回原因提示；Playwright 端到端 11/11 通过） |
