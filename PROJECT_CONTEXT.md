...
### 2026-09-23 pullFromCloud 修复（spawnSync EBUSY）
- 现象：系统设置页"拉取云端数据"报 `spawnSync python EBUSY`
- 根因：spawnSync 默认 pipe 模式创建管道，服务以重定向方式启动时 stdio 句柄异常导致失败
- 修复：改用文件句柄接管输出 `stdio: ['ignore', logFd, logFd]`；Python 解释器改为按优先级依次尝试（venv路径 → 硬编码路径 → PATH）
- 依赖：需 paramiko，venv 路径 `C:\Users\PC\.workbuddy\binaries\python\envs\default\Scripts\python.exe`
- 验证：拉取成功返回"共 34 个用户、49 条记录"
...
- **培训项目命名**：`培训名·第N期`，例：`客涨价培训战营·第1期`
  （用"·第N期"后缀区分期次，后续按项目名筛选即可汇总全部期次）
- **每条记录粒度**：1人 × 1期 = 1条记录
- **状态**：设为「已通过」（表示已批准参加，等待填写内容）
- **课堂一页纸字段映射**：
...
- **录入方式**（二选一）：
  - 方案A（推荐）：HR 批量建记录 → 员工登录自助填写三栏内容并提交
