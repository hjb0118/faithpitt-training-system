...
- [2026-09-23](logs/2026-09-23.md) 全量代码审查；安全加固 3 项（静态文件任意下载 / register 越权 / SSH 密码硬编码）；环境修复（Node 22）；建立 MEMORY.md + logs/ 日志制度；数据操作（员工密码统一 123456、删「高管领导力」25 条并同步云端）；bug 修复 x2（状态机缺评审退回路径、pullFromCloud 不做云端 WAL checkpoint）+ 张磊 R52 退回 + 两端数据归一
